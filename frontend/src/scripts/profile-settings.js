...
/* NAV */
const hb=document.getElementById('hamburgerBtn'),mm=document.getElementById('mobileMenu'),bd=document.getElementById('backdrop');
let navOpen=false;
const openNav=()=>{navOpen=true;hb.classList.add('open');mm.classList.add('open');bd.classList.add('visible');document.body.style.overflow='hidden';};
const closeNav=()=>{navOpen=false;hb.classList.remove('open');mm.classList.remove('open');bd.classList.remove('visible');document.body.style.overflow='';};
hb.addEventListener('click',e=>{e.stopPropagation();navOpen?closeNav():openNav();});
bd.addEventListener('click',closeNav);
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeNav();document.querySelectorAll('.edit-overlay.open').forEach(o=>o.classList.remove('open'));document.body.style.overflow='';}});
...
document.getElementById('fieldName').addEventListener('input',function(){
    ...
    if(n){
        ...
        const ini=n.split(/\s+/).map(w=>w[0]).join('').substring(0,2).toUpperCase();
